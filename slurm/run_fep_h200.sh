...
export SLURM_ACCOUNT="${SLURM_ACCOUNT:-hria}"

: "${SLURM_TIMELIMIT:?Set SLURM_TIMELIMIT in the environment or .env}"

# sbatch --gres gpu:$NUM_GPUS -A phd -p dgxa100 --time=$SLURM_TIMELIMIT --ntasks 1 --cpus-per-task 8 --mem-per-cpu 8G $SCRIPT
sbatch --export=ALL,NUM_GPUS="$NUM_GPUS",ENV_NAME="$ENV_NAME",HF_HOME="$HF_HOME" --gres="gpu:${NUM_GPUS}" -A "$SLURM_ACCOUNT" -p h200 --time="$SLURM_TIMELIMIT" --ntasks 1 --cpus-per-task 8 --mem-per-cpu 8G "$SCRIPT"
