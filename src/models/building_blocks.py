...
class SwiGLUFFN(nn.Module):
    ...
    def __init__(self, args, in_features, hidden_features = None, out_features = None, bias = True, drop = 0., gaussian_noise = 0.0):
        super().__init__()
        self.args = args
        self.drop = drop
...
class MultiHeadAttention(nn.Module):
    ...
    def apply_attention(self, q, k, v, mask = None, apply_pos_emb = False):
        ...
        out = F.scaled_dot_product_attention(q, k, v, attn_mask = mask, scale = self.scale, dropout_p = self.attn_drop if self.training else 0.0)
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        for block_idx in range(depth):
            if block_idx in context_position and has_context:
                print('::: Adding cross-attention at layer', block_idx)
            ...
            self.layers.append(
                nn.ModuleList([
                    ...
                    CrossAttention(
                        ...
                        rotary_emb = self.rotary_emb,
                        nheads = nheads,
                        attn_drop = cross_attn_dropout,
                        qkv_bias = False,
                        qk_norm = True,
                    ) if has_context and block_idx in context_position else None,
                    SwiGLUFFN(args, in_features = dmodel, hidden_features = 4 * dmodel, out_features = dmodel, gaussian_noise = gaussian_noise, drop = dropout, bias = True)
                ]))
