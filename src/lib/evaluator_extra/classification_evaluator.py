...
class AcumenClassificationEvaluator(AcumenEvaluator):
    ...
    def __init__(self, args, model, evaluator_args):
        super().__init__(args, model, evaluator_args)
    ...
    def evaluate(self, save = True):
        ...
        for head in self.args.heads:
            ...
            results += [
                ...
                Metric(name = f'{head.name}@precision', value = precision_score(y_true, y_pred_am, average = self.evaluator_args.average_kind), monotonicity = ['instant', 'up']),
                Metric(name = f'{head.name}@recall', value = recall_score(y_true, y_pred_am, average = self.evaluator_args.average_kind), monotonicity = ['instant', 'up']),
                Metric(name = f'{head.name}@f1', value = f1_score(y_true, y_pred_am, average = self.evaluator_args.average_kind), monotonicity = ['instant', 'up']),
            ]

        # Maybe a result serializer or something?
        if save:
            with open(f"results/{self.args.output_dir}/{self.args.group}:{self.args.name}_results.json", 'w') as f:
                json.dump(results, f)
